        ...
        for file_name in files:
            if os.path.splitext(file_name)[-1] == '.csv': # Depends on file type
                with open(file_name, 'r',encoding = 'ISO-8859-1') as f:
                    ...
                    #arff file write
                    with open('./arff_files/' + file_name + '.arff','w',newline='') as f: # make arff file format
...
''')
                        for index, line in enumerate(roc):
                            ...
                            f.write(str(roc[index])+","+str(mcr[index])+","+str(std[index]) + "," + str(iran[index]) + "," + str(kur[index]) +  "," + str(rss[index]) + "," +str(rms[index]) + "," +label[index]+"\n")
